Intuition : Well well, the question is quite the tough one today.So make your focus lasersharp and let's jump in. The question is return true or false if the given n is power of two or not. The simple approach that comes to mind after reading this, is our classsic reminder and divident menthod that we used for finding odd and even . The question is easy but can be solved in tooo many ways .Try to understand all the solution as it may help in interviews where there can be restriction of using certain methods
...
6. LMAO method
    ...
    Hint : Constraints[-2 ^ 31 <= n <= 2 ^ 31 - 1] also 2 ^
    30 is 1073741824 class Solution
{
public:
    bool isPowerOfTwo(int n)
    {
